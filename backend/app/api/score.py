...
def get_user_scores(user_id: UUID, db: Session = Depends(get_db)):
    # presentationsをuser_idで絞って、そのpresentation_idに紐づくfeedbacksをJOIN
    results = (
        db.query(models.Presentation, models.Feedback)
        .join(models.Feedback, models.Presentation.id == models.Feedback.presentation_id)
    ...
    score_list = []
    for presentation, feedback in results:
        score_list.append(
            schemas.ScoreRecord(
